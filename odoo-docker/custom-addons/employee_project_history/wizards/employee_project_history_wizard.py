...
class EmployeeProjectHistoryWizard(models.TransientModel):
    _name = 'employee.project_history.wizard'
    ...
    def action_preview(self):
        ...
        body = ''
        column_header = [_('Employee'), _('Name'), _('Members'), _('Start date'),
                         _('End Date'), _('Status')]
    ...
    def action_export(self):
        ...
        filename = 'employee_project_history_report' + '.xls'
        workbook = xlwt.Workbook(encoding="UTF-8")
        worksheet = workbook.add_sheet('Employee Project History')
    ...
    def _write_sheet(self, worksheet, data_set):
        ...
        col = 1
        worksheet.write_merge(row, mer_row, col, col, 'Name', header_style)
        col = 2
        worksheet.write_merge(row, mer_row, col, col, 'Members', header_style)
        col = 3
    ...
    def generate_data_all(self):
        ...
        # with ORM
        project_history_objs = self.env['employee.project_history'].search(where_con)
        for project_history_obj in project_history_objs:
            row_val = {
                'employee': project_history_obj.employee_id.name,
                'name': project_history_obj.name,
                'no_of_members': project_history_obj.no_of_members,
                'start_date': project_history_obj.start_date,
                'end_date': project_history_obj.end_date,
                'status': project_history_obj.status,
            }
            list_of_all_records.append(row_val)
        # with SQL
        # sql = """
        #     SELECT he.name as employee, name, no_of_members,
        #     start_date, end_date, status
        #     FROM employee_project_history as eph
        #     LEFT JOIN hr_employee as he on eph.employee_id = he.id
        #     %s
        # """ % (str(condition_str))
